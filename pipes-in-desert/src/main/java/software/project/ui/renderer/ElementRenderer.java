...
public class ElementRenderer {
    ...
    public void drawPumps(Graphics2D g, List<Pump> pumps, Grid grid) {
        ...
        Sprite brokenSprite = sm.getSprite(Sprites.PUMP_BROKEN);
        ...
        for (Pump pump : pumps) {
            ...
            if (pump.isBroken()) {
                // Draw broken pump sprite (static, no fan)
                if (brokenSprite != null) {
                    brokenSprite.drawCentered(g, center.x, center.y, grid.getTileSize(), 0);
                }
                continue;
            }

            // Normal pump rendering (intact)
            int currentFlow = pump.getCurrentFlowingWater();
            int maxFlow = GameConfig.PUMP_MAX_FLOW_PER_TICK;
            int percentFlow = (int) ((currentFlow / (float) maxFlow) * 100);
            if (percentFlow > 100) {
